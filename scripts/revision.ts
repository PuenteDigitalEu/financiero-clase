/**
 * scripts/revision.ts — Revisión diaria de mercado. Pega la capa de vigilancia con el mundo real:
 *
 ...
 *   2. Lee de Supabase las reglas activas y los clientes con su perfil, su modo y su análisis.
 *   3. Decide con `detectarEventos` + `clientesAfectados` (importados de src/lib/alertas/, sin
 *      duplicar la lógica).
 *   4. Registra el evento y una alerta por cliente afectado apoyándose en los UNIQUE de la
 *      migración 0002: reejecutar el script no crea nada nuevo.
 *   5. Envía un correo real (visual, con el porcentaje destacado y el descargo legal siempre
 *      presente) SOLO por cada alerta recién insertada cuyo cliente tenga `avisar_cliente`.
 *   6. Imprime un resumen en JSON por stdout.
 *
 * Ejecutar:  pnpm revision       (carga .env.local si existe; en cron, usa el entorno del proceso)
 *
...
import { clientesAfectados, detectarEventos } from '../src/lib/alertas/evaluar';
import { mensajeInterno } from '../src/lib/alertas/redactar';
import { DESCARGO_LEGAL } from '../src/lib/alertas/umbrales';
import type {
  BandaProbabilidad,
  ClaseActivo,
  ClienteConPlan,
...
const SUPABASE_URL = requerido('NEXT_PUBLIC_SUPABASE_URL');
const SERVICE_ROLE_KEY = requerido('SUPABASE_SERVICE_ROLE_KEY');

// El correo al cliente está APAGADO por defecto (decisión 2026-08-31: "sin correo al cliente aún").
// El script registra eventos y alertas igual; solo se salta el envío. Para activarlo hay que poner
// REVISION_ENVIAR_CORREO_CLIENTE=true y tener RESEND_API_KEY (y un dominio verificado en Resend).
const ENVIAR_CORREO_CLIENTE = process.env.REVISION_ENVIAR_CORREO_CLIENTE === 'true';
const RESEND_API_KEY = ENVIAR_CORREO_CLIENTE ? requerido('RESEND_API_KEY') : '';
const EMAIL_FROM =
...
async function descargarCierresSP500(): Promise<Cierre[]> {
  ...
  const res = await fetch(url, {
    headers: {
      ...
      'User-Agent':
        'Mozilla/5.0 (revision-diaria; landing-agente-financiero) AppleWebKit/537.36',
...
async function leerReglasActivas(): Promise<ReglaAlerta[]> {
...
async function leerSerie(desdeIso: string): Promise<Observacion[]> {
  const { data, error } = await supabase
...
async function leerClientesConPlan(): Promise<ClientePlanContacto[]> {
...
async function main(): Promise<void> {
  ...
  const resumenEventos: Array<Record<string, unknown>> = [];
  let eventosNuevos = 0;
  ...
  for (const evento of eventos) {
    ...
    for (const afectado of afectados) {
      ...
      try {
        await enviarCorreo(contacto.email, contacto.nombre, evento);
        await supabase
          .from('alertas')
  ...
  const resumen = {
    ...
    correo_cliente_activo: ENVIAR_CORREO_CLIENTE,
  ...
  process.exitCode = correosFallidosTotal > 0 ? 1 : 0;
}
