...
> Fuente de criterio: **`docs/criterio/politica-de-inversion.md`**. Este archivo NO contiene criterio financiero: solo define CÓMO aplicarlo. Si un valor o umbral aparece aquí, es un error — debe vivir en la política.
> Principio rector: el motor aplica la política literalmente. **Caso sin regla escrita → no se improvisa: se marca "pendiente para la reunión" y se lista en la sección de pendientes del informe.**
...
1. **Leer** la ficha completa y `docs/criterio/politica-de-inversion.md`. Si falta cualquiera de los dos, decirlo y detenerse.
...
- Rangos dentro de un valor (p. ej. «300-400 €») → usar el extremo prudente según la dirección de sesgo de la política (sección de sesgo en datos estimados) y declararlo.
- **Hueco conocido:** nuestra ficha no incluye `patrimonio_distribucion` (composición por clase de activo de lo ya invertido). Mientras no se añada a la entrevista, la sección de transición del patrimonio (política §8) queda siempre «pendiente» para clientes con `patrimonio_invertido` > 0 — no se inventa la composición.
...
| **Renta de cartera** | `objetivo_proposito` describe una renta mensual/anual a vivir, sin negocio propio de por medio (p. ej. «vivir de las rentas», jubilación anticipada) | Convertir con política §7 (tasa de retirada según horizonte de la retirada). Documentar la conversión en el informe. |
| **Renta de negocio propio** | `objetivo_proposito` o `situacion_laboral` indican que la renta procede de una actividad/negocio propio del cliente | **No convertir** (política §7). La meta no se persigue vía cartera; el informe lo dice explícitamente. La cartera se analiza por su papel real (colchón, diversificación), no como vehículo de la meta. |
...
- **Completo:** las 6 variables críticas de la política (ingresos y gastos esenciales, deudas, liquidez y colchón, patrimonio invertido, objetivo y plazo, capacidad y tolerancia al riesgo) presentes, aunque alguna sea `estimado` → diagnóstico + propuesta preliminar ejecutable.
...
- Flujo libre primero: si ≤ 0, aplica el plan de estabilización de la política y no sigue con el resto de esta lista.
...
- **Fuera de alcance por ahora:** la política define un cálculo de probabilidad por Monte Carlo. No está implementado. Mientras tanto, esa cifra queda «pendiente — requiere motor de simulación no implementado»; no se inventa una probabilidad.
...
| C11 | `aportacion_mensual_actual` = 0 con flujo libre > 0 | Hecho descriptivo, sin juicio; la propuesta parte de la política con normalidad. |
...
8.  Prioridades aplicadas     (orden de la política sobre este cliente, paso a paso)
...
## Parte C — Control
13. Trazabilidad             (cada cifra → dato de ficha o sección de la política)
14. Pendientes para la reunión (los de la ficha + los generados por el motor)
```

- Parte A describe, nunca valora ni recomienda.
- Parte B es propuesta para el asesor: puede proponer cifras y distribuciones, siempre dentro de la política y rotulada como borrador. Nunca productos concretos ni lenguaje dirigido al cliente («deberías»).
